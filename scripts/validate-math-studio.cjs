...
function loadSite(storage=new Map(),broken=false){
 ...
 for(const f of ['data.js','question-bank.js','practice.js','inquiry.js','app.js'])vm.runInContext(fs.readFileSync(path.join(root,'public','math-studio',f),'utf8'),ctx,{filename:f});
...
for(const l of lessons){const pool=bank.pool(l.id);assert(pool.length>=30,l.id+' pool');assert.equal(new Set(pool.map(q=>q.key)).size,pool.length);assert(pool.some(q=>q.type==='written'));assert.equal(new Set(pool.map(q=>q.family)).size,8);
 ...
 for(let round=0;round<3;round++){const out=bank.select(c,history);assert.equal(out.questions.length,15,l.id+' / level '+level);assert.equal(new Set(out.questions.map(q=>q.key)).size,15);for(const q of out.questions){assert.equal(q.lessonId,l.id);assert.equal(q.grade,l.grade);assert(level==='mix'||q.level===+level);if(!history[q.key]){}else assert(out.repeats>0);if(q.type==='choice'){const original=pool.find(p=>p.key===q.key);assert.equal(q.options[q.answer],original.options[original.answer]);}history[q.key]=round+1;}if(!first)first=out.questions;}
 }
...
assert(s.app.innerHTML.includes('客觀題：答對 9 / 已答 9 題'));assert(s.app.innerHTML.includes('書面題已展開評量 1 題'));
