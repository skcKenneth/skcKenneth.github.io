...
2. **Constrained Hierarchical Clustering** where constellation groups must satisfy multiple simultaneous constraints—maximum angular diameter $ D_{max} \leq 50\degree $ ensuring they fit within typical field of view, minimum cluster size $ n_{min} \geq 3 $ ensuring identifiability, minimum separation $ \delta_{min} \geq 10\degree $ ensuring distinctness, and minimum bright stars $ n_{bright} \geq 1 $ ensuring visibility—creating a multi-objective optimization problem
...
2. **Multi-Constraint Optimization**—the clustering simultaneously satisfies maximum diameter $ D_{max} \leq 50\degree $, minimum size $ n_{min} \geq 3 $, minimum separation $ \delta_{min} \geq 10\degree $, and minimum bright stars $ n_{bright} \geq 1 $, requiring sophisticated post-processing algorithms that merge small clusters and split oversized clusters while maintaining separation
...
Stars exist on the celestial sphere, a two-dimensional surface embedded in three-dimensional space. This creates fundamental differences from planar clustering problems. Consider two stars separated by 30\degree on the celestial sphere. In Euclidean space, we might compute their distance as a straight line. But on a sphere, the correct distance is the **great-circle distance**—the angular separation along the sphere's surface.
...
**Constraint 1: Maximum Diameter** $ D_{max} \leq 50\degree $ ensures that constellations fit within typical field of view for naked-eye observation. A constellation spanning 80\degree would be impossible to view entirely, defeating the purpose of grouping.
...
**Constraint 3: Minimum Separation** $ \delta_{min} \geq 10\degree $ ensures that constellations are distinct. If two constellations have centroids only 5\degree apart, they might be confused.
...
The system operates under realistic constraints including angular diameter limits ensuring constellations fit within typical field of view (50\degree maximum), minimum size requirements ensuring identifiability (3+ stars), separation requirements ensuring distinctness (10\degree minimum between centroids), brightness requirements ensuring visibility (at least 1 bright star per constellation), and observational constraints where all stars must be visible to naked eye (Vmag ≤ 6.5).
...
**Irregular Shapes**: Traditional constellations have highly irregular boundaries, some spanning over 100\degree in one dimension while being narrow in another. This makes them difficult to visualize and identify.
...
The first mathematical challenge is representing stars on the celestial sphere. Stars are observed in spherical coordinates: Right Ascension (RA) $ \alpha \in [0, 360\degree] $ and Declination (DE) $ \delta \in [-90\degree, +90\degree] $. For clustering and geometric operations, we convert to Cartesian coordinates on the unit sphere:
...
**Arrow Pattern**: High aspect ratio ($ \rho > 2.0 $), moderate compactness ($ 6\degree < \bar{d}_{centroid} < 25\degree $), 4+ stars. Arrows are elongated shapes pointing in a direction.

**Serpent Pattern**: Very high aspect ratio ($ \rho > 3.0 $), high compactness ($ 12\degree < \bar{d}_{centroid} < 35\degree $), 6+ stars. Serpents are long, sinuous shapes.

**Belt Pattern**: Moderate aspect ratio ($ 1.0 < \rho < 2.0 $), moderate compactness ($ 5\degree < \bar{d}_{centroid} < 20\degree $), 3+ stars. Belts are elongated but not as extreme as arrows.

**Chain Pattern**: High aspect ratio ($ \rho > 2.0 $), variable compactness, 4+ stars. Chains are connected sequences of stars.

**Cluster Pattern**: Low aspect ratio ($ \rho < 1.3 $), low compactness ($ \bar{d}_{centroid} < 8\degree $), 4+ stars. Clusters are compact, roughly circular groups.
...
Constellation diameters range from **18.2\degree to 49.0\degree**, with all values satisfying the $ D \leq 50\degree $ constraint. The distribution shows:
- **Mean diameter**: 28.5\degree
- **Median diameter**: 27.1\degree
- **Standard deviation**: 6.8\degree

**Mathematical Interpretation**: The diameter distribution reflects the clustering algorithm's effectiveness. Most constellations have diameters in the 20-35\degree range, providing good balance: large enough to be identifiable (not too fragmented) but small enough to fit in field of view. The maximum diameter of 49.0\degree approaches but doesn't exceed the constraint, demonstrating that the splitting algorithm successfully enforces limits.

**Compactness Analysis**: Average compactness (mean distance to centroid) ranges from **6.1\degree to 14.5\degree**, with mean 8.2\degree. This indicates that stars within constellations are reasonably clustered around centroids, supporting identifiability. High compactness (low values) indicates tight clusters, while low compactness (high values) indicates more spread-out groups.
...
**Minimum Separation**: $ \delta_{min} = 8.3\degree $ - the closest pair of constellation centroids are 8.3\degree apart. This is slightly below the target $ \delta_{min} \geq 10\degree $, but represents a trade-off with other constraints. In dense regions, enforcing strict 10\degree separation might require merging constellations, reducing identifiability.

**Average Separation**: $ \bar{\delta} = 45.2\degree $ - constellation centroids are on average 45.2\degree apart. This indicates good distribution across the celestial sphere, with constellations well-spaced.

**Separation Compliance**: 87.3% of constellation pairs have separation $ \geq 10\degree $. The 12.7% of pairs with separation $ < 10\degree $ occur primarily in dense regions (Milky Way plane) where star density creates natural clustering that conflicts with strict separation requirements.
...
5. Validates error < 5\degree threshold

**Result**: The validation successfully finds Polaris with angular error < 3\degree, demonstrating that the coordinate transformations and spherical geometry calculations are correct. This validates the mathematical foundation for all subsequent clustering operations.
