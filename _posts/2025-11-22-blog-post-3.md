...
4. **Optimal Cluster Number Determination** where silhouette score maximization $s(i) = \frac{b(i) - a(i)}{\max(a(i), b(i))}$ across candidate cluster numbers $K \in \lbrack 40, 120 \rbrack$ identifies the optimal balance between separation and compactness
...
4. **Silhouette-Based Optimal K**—the optimal number of constellations $K^*$ is determined by maximizing silhouette score $s(i) = \frac{b(i) - a(i)}{\max(a(i), b(i))}$ across candidate values $K \in \lbrack 40, 120 \rbrack$, where $a(i)$ is average distance to same-cluster stars and $b(i)$ is minimum average distance to other clusters, providing mathematically rigorous cluster number selection
...
Mathematically, if two stars have Cartesian coordinates $\mathbf{p}_1 = \lbrack x_1, y_1, z_1 \rbrack$ and $\mathbf{p}_2 = \lbrack x_2, y_2, z_2 \rbrack$ on the unit sphere (where $\lVert\mathbf{p}_i\rVert = 1$), the great-circle distance is:
...
The first mathematical challenge is representing stars on the celestial sphere. Stars are observed in spherical coordinates: Right Ascension (RA) $\alpha \in \lbrack 0, 360^\circ \rbrack$ and Declination (DE) $\delta \in \lbrack -90^\circ, +90^\circ \rbrack$. For clustering and geometric operations, we convert to Cartesian coordinates on the unit sphere:
...
**Numerical Stability**: The $\arccos$ function requires its argument to be in $\lbrack -1, 1 \rbrack$. Due to floating-point errors, dot products might slightly exceed this range. The framework clips values: $\mathbf{p}_i \cdot \mathbf{p}_j \leftarrow \text{clip}(\mathbf{p}_i \cdot \mathbf{p}_j, -1, 1)$ before computing $\arccos$.
...
The framework determines the optimal number of constellations $K^*$ by maximizing silhouette score across candidate values $K \in \lbrack 40, 120 \rbrack$ with step size 5.
...
Each pattern recognition has a confidence score $c \in \lbrack 0, 1 \rbrack$ based on how well the constellation's features match the pattern template: $c = 0.3 \cdot c_{aspect} + 0.3 \cdot c_{compactness} + 0.4 \cdot c_{structure}$ where:
...
4. **Optimal K Search**: For each candidate $K \in \lbrack 40, 120 \rbrack$:
...
for label in unique_labels:
    ...
    n_bright = sum(vmag\lbrack cluster_mask \rbrack < BRIGHT_THRESHOLD)
    
    if cluster_size < MIN_SIZE or n_bright < MIN_BRIGHT:
        # Find nearest valid cluster
        cluster_coords = coords_xyz\lbrack cluster_mask \rbrack
        cluster_weights = weights\lbrack cluster_mask \rbrack
        centroid = weighted_spherical_mean(cluster_coords, cluster_weights)
        
        min_dist = inf
        nearest_label = None
        for other_label in other_labels:
            if size\lbrack other_label \rbrack >= MIN_SIZE:
        ...
        # Merge
        labels\lbrack cluster_mask \rbrack = nearest_label
```
...
for label in unique_labels:
    cluster_mask = labels == label
    cluster_distances = distance_matrix\lbrack cluster_mask \rbrack\lbrack :, cluster_mask \rbrack
    diameter = max(cluster_distances)
    
    if diameter > MAX_DIAMETER:
        # Extract sub-cluster
        cluster_indices = where(cluster_mask)\lbrack 0 \rbrack
        ...
        for sub_label in unique(sub_labels):
            ...
            global_indices = cluster_indices\lbrack sub_mask \rbrack
            if sub_label == 1:
                labels\lbrack global_indices \rbrack = label
            else:
                max_label += 1
                labels\lbrack global_indices \rbrack = max_label
```
...
def extract_features(coords_xyz, weights, vmag):
    ...
    # Compactness
    dists_to_centroid = \lbrack arccos(clip(dot(coord, centroid), -1, 1)) 
                         for coord in coords_xyz \rbrack
...
def recognize_pattern(features, templates):
    ...
    for pattern_name, template in templates.items():
        # Check constraints
        if features\lbrack 'n_stars' \rbrack < template\lbrack 'min_stars' \rbrack:
            continue
        
        ar_min, ar_max = template\lbrack 'aspect_ratio_range' \rbrack
        comp_min, comp_max = template\lbrack 'compactness_range' \rbrack
        
        if (ar_min <= features\lbrack 'aspect_ratio' \rbrack <= ar_max and
            comp_min <= features\lbrack 'compactness' \rbrack <= comp_max):
            
            # Compute confidence
            ar_score = 1 - abs(features\lbrack 'aspect_ratio' \rbrack - (ar_min+ar_max)/2) / (ar_max-ar_min)
            comp_score = 1 - abs(features\lbrack 'compactness' \rbrack - (comp_min+comp_max)/2) / (comp_max-comp_min)
...
The framework determined the optimal number of constellations as **$K^* = 76$** through silhouette score maximization. The silhouette score analysis across candidate values $K \in \lbrack 40, 120 \rbrack$ revealed a clear maximum at $K = 76$, with $S(76) \approx 0.35$ (weighted average).
