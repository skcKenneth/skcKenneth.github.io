...
- Evaluate environmental benefits (CO₂ avoided) and system resilience under extreme weather scenarios.
...
**The Results**:  
- **150% improvement** in demand forecasting accuracy (R² from 0.260 to 0.650+)
...
- **79+ tonnes CO₂** prevented annually through system optimization
...
$$
\begin{align*}
\text{temp\_weekend} &= \text{temp\_normalized} \times \text{is\_weekend} \\
\text{temp\_hour} &= \text{temp\_normalized} \times \text{hour\_sin}
\end{align*}
$$
...
**Results**: Achieved **$R^2 > 0.65$** across 85% of stations, with feature importance analysis revealing that weather and temporal cyclical features contributed 65% of forecasting accuracy.
...
$$
\text{distance}_{ij} = \sqrt{\left[(\text{lat}_i - \text{lat}_j)² + (\text{lon}_i - \text{lon}_j)²\right]} × 111,000
$$
...
$$
\operatorname{system\_efficiency} = \Sigma(bikes\_moved\_k) / \Sigma(distance\_k)
$$
...
$$
\text{Maximize: Revenue} = \Sigma(price_i × demand_i(price_i))
\text{Minimize: Inequity} = Gini_coefficient(monthly_costs)
$$

**Demand Response Model**:

$$
\text{demand}_i(\text{price}_i) = \text{base\_demand}_i × \left(\frac{\text{price}_i}{\text{current\_price}_i}\right)^{\text{elasticity}_i}
$$
...
$$
\text{CO₂\_savings} = \text{annual\_trips} × \text{substitution\_rate} × \text{avg\_distance} × \text{emission\_factor}
$$
...
|-----------|----------|----------|-------------|--------|
| **Demand Forecasting** | $\operatorname{R}^2 = 0.26$ | $\operatorname{R}^2 = 0.65+$ | **+150%** | Reliable planning foundation |
...
- **Exceptional Predictive Power**: Models explain $87\%+$ of demand variance
...
- **Mean R²: 0.871** - Demonstrates $87\%$ variance explanation
