...
For a linear car-following model $\dot{v}_i = f(s_i, v_i, \Delta v_i)$, string stability is determined by the **transfer function** $G(s)$ that relates the input (leader's velocity) to the output (follower's velocity). The system is string-stable if $|G(j\omega)| < 1$ for all frequencies $\omega$.
