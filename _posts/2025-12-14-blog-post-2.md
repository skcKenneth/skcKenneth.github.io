...
In the last post, we embraced "good enough." This sequel goes deeper into the math of how heuristics move across rugged **fitness landscapes**—the invisible mountains where every coordinate encodes a solution and its height is the objective value.
...
For combinatorial problems, "neighbors" define what a single move can change:

- Traveling Salesman: swap two cities, reverse a segment, or 2-opt/3-opt rewires.
- Scheduling: swap time slots of two tasks; move one task to a new slot.

Richer neighborhoods reduce trap risk but raise per-move cost. Heuristic design is a balance: broader moves explore better; cheaper moves iterate faster.
