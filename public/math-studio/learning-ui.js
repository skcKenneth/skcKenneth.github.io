...
const LearningUI = (() => {
  ...
  function compareLauncher(lesson,key){
    ...
    return `<div class="comparison-launcher" data-comparison-key="${esc(key)}">${btn(s.open?'收起比較題':'比較一題容易混淆的',`id="toggle-${esc(key)}" data-compare-toggle="${esc(key)}" aria-expanded="${s.open}"`)}${s.open?comparePanel(key,s):''}</div>`;
  }
  ...
  function redraw(focus){const y=window.scrollY,active=focus||document.activeElement?.id;render();window.scrollTo(0,y);if(active)document.getElementById(active)?.focus({preventScroll:true});}
  ...
  function bind(root){
    ...
    root.addEventListener('click',e=>{
      ...
      if(b.dataset.compareNext){const s=records.get(b.dataset.compareNext);s.attempts.at(-1).kind=s.kind;s.attempts.push({seed:s.attempts.at(-1).seed+1,prediction:'',left:'',right:'',checked:false});redraw('prediction-'+b.dataset.compareNext);}
      if(b.dataset.supportNext){const s=exerciseState(b.dataset.supportNext);s.seed+=313;s.draft='';s.checked=false;redraw('support-'+b.dataset.supportNext);}
    });
    root.addEventListener('submit',e=>{
      ...
      redraw(key?'support-feedback-'+key:(records.get(compare).attempts.at(-1).error?'comparison-error-':'comparison-feedback-')+compare);
    });
  }
